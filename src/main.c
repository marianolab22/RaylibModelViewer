...
int main(void)
{
    ...
    Model model = LoadModel("Carrillo.glb");       // Load model
    Texture2D texture = LoadTexture("Carrillo_texture.png");// Load model texture
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
       ...
       //----------------------------------------------------------------------------------
        ...
        DrawModel(model, (Vector3) { 0, 0, 0 }, 2.0f, RED); // Dibujar el modelo con la textura
        rlPopMatrix();
